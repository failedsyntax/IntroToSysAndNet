...
// Declare the Mutex'd variable
int calculationCount = 0;
pthread_mutex_t calcCountLock;
...
void *multiply(void *arg) {
    pthread_mutex_lock(&calcCountLock);
    calculationCount++;
    ...
    pthread_mutex_unlock(&calcCountLock);
    pthread_exit(NULL);
}
...
int main() {
    ...
    pthread_mutex_init(&calcCountLock, NULL);
    ...
    printf("Completed Calculation Count: %d", calculationCount);
    pthread_mutex_destroy(&calcCountLock);
